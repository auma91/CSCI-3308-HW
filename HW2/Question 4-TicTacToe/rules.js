...
function begin_play(){
	if(document.getElementById("player1_id").value.length == 0 || document.getElementById("player2_id").value.length == 0){
		 return window.alert("Must enter two player names!")
	}
	else{
		//give players their respective x
		...
		if( whose_move() == 0){
 		...
 		document.getElementById("turn_info").innerHTML = "Turn of " + y ;
	}
}
...
function play() {
	...
	var player;
	if(!started){
		window.alert("Game hasn't started, please enter names and click play")
		return
	}
	if( whose_move() == 0){
		player = "0"
	...
	if(board_state[indicie] != -1){
		 window.alert("Choose some other spot, this one is already used.")
		 return
	}
	else if(valid){
		document.getElementById(table_ids[indicie]).innerHTML = player
		//table_ids[indicie]= player
		board_state[indicie] = whose_move;
		document.getElementsByClassName(board_state[indicie]).value = whose_move()
	}
	toggle_move()
	check_winner(player);
	document.getElementById("turn_info").innerHTML = "Turn of " + player;
}

function check_winner(player_sign){
	if(table_ids[0] == player_sign && table_ids[1] == player_sign&&table_ids[2] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
	}
	if(table_ids[0] == player_sign && table_ids[3] == player_sign &&table_ids[6] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
	 	started = false
	 return
 	}
	if(table_ids[0] == player_sign && table_ids[4] == player_sign &&table_ids[8] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
 	}
	if(table_ids[1] == player_sign && table_ids[4] == player_sign &&table_ids[7] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
	}
	if(table_ids[3] == player_sign && table_ids[4] == player_sign &&table_ids[5] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
 	}
	if(table_ids[2] == player_sign && table_ids[5] == player_sign &&table_ids[8] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
	}
	if(table_ids[6] == player_sign && table_ids[7] == player_sign &&table_ids[8] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
	started = false
	 return
	}
	if(table_ids[6] == player_sign && table_ids[4] == player_sign &&table_ids[2] == player_sign){
		window.alert(player_sign +" Won!!! (note you have been locked out of the game press the restart button)")
		started = false
	 return
	}
